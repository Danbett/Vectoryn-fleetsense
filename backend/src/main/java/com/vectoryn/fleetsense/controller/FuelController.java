...
public class FuelController {
    ...
    public ResponseEntity<?> summary(
        ...
        var rows = db.queryForList(
            ...
            "      THEN (p.attributes::jsonb->>'totalDistance')::numeric ELSE NULL END) - " +
            "    MIN(CASE WHEN p.attributes IS NOT NULL " +
            "      THEN (p.attributes::jsonb->>'totalDistance')::numeric ELSE NULL END)" +
            "  ) / 1000.0, 0)::numeric(10,2) as distance_km, " +
            "  COALESCE((" +
            "    MAX(CASE WHEN p.attributes IS NOT NULL " +
            "      THEN (p.attributes::jsonb->>'hours')::numeric ELSE NULL END) - " +
            "    MIN(CASE WHEN p.attributes IS NOT NULL " +
            "      THEN (p.attributes::jsonb->>'hours')::numeric ELSE NULL END)" +
            "  ) / 3600000.0, 0)::numeric(10,2) as engine_hours " +
            "FROM tc_devices d " +
    ...
    public ResponseEntity<?> deviceHistory(
        ...
        var buckets = db.queryForList(
            "SELECT date_trunc('hour', fixtime) as hour, " +
            ...
            "GROUP BY date_trunc('hour', fixtime) ORDER BY hour ASC",
            id);

        return ResponseEntity.ok(Map.of("data", buckets, "device", check.get(0)));
    }
    ...
    public ResponseEntity<?> events(
        ...
        var events = db.queryForList(
            ...
            "  GROUP BY p.deviceid, d.name, date_trunc('hour', p.fixtime) " +
            "), deltas AS (" +
            "  SELECT *, max_battery - LAG(max_battery) " +
            "    OVER (PARTITION BY deviceid ORDER BY hour) as delta FROM hourly " +
            ") " +
            "SELECT deviceid, name, hour, min_battery, max_battery, delta, " +
            "  CASE WHEN delta > 5 THEN 'charge' WHEN delta < -5 THEN 'discharge' " +
            "  ELSE 'normal' END as event_type " +
            "FROM deltas WHERE ABS(COALESCE(delta,0)) > 5 " +
    ...
    public ResponseEntity<?> tank(@PathVariable long id, HttpServletRequest req) {
        ...
        var device = db.queryForList(
            ...
            "p.attributes, p.fixtime, p.speed FROM tc_devices d " +
